...
queryrealdata = "select P.BarCode,SPD.PositionCode,sum(SPD.Qty) AS '总数量' from V_t_OutOrder O " #根据计划出库时间
...
queryrealdata += "left join V_t_Product P on P.ProductCode = SPD.ProductCode "
queryrealdata += "where SPO.Abc_Type = 'A' and SPO.StorageCode = 'ALOG-0003-03' AND O.PlanOutTime >= {} "
queryrealdata += "AND O.PlanOutTime <= {} and O.CancelbyWLB = 0   group by P.BarCode,SPD.PositionCode "
#and left(SPD.PositionCode,2) <> '42'
queryorderdata = "select P.BarCode,SPD.PositionCode,sum(SPD.Qty) AS '总数量' from V_t_OutOrder O " #根据订单流入时间
...
queryorderdata += "left join V_t_Product P on P.ProductCode = SPD.ProductCode "
queryorderdata += "where SPO.Abc_Type = 'A' and SPO.StorageCode = 'ALOG-0003-03'   AND O.OrderTime >= {} "
queryorderdata += "AND O.OrderTime <= {} AND O.PlanOutTime >= {}  AND O.PlanOutTime <= {} and O.CancelbyWLB = 0 group by P.BarCode,SPD.PositionCode "

queryorderinflowdata = "select P.BarCode,SQ.PositionCode,sum(OD.Qty) AS '总数量' from V_t_OutOrder O "
queryorderinflowdata += "left join V_t_OutOrderDetail OD on O.OutOrderNo = OD.OutOrderNo "
queryorderinflowdata += "left join V_t_Product P on OD.ProductCode = P.ProductCode "
queryorderinflowdata += "left join V_t_Storage_Qty SQ on P.ProductCode = SQ.ProductCode "
queryorderinflowdata += "left join V_t_Storage_Position SPO on SQ.PositionCode = SPO.PositionCode "
queryorderinflowdata += "where SPO.Abc_Type = 'A' and SPO.StorageCode = 'ALOG-0003-03' AND O.OrderTime >= {}  "
queryorderinflowdata += "AND O.OrderTime <= {}  AND O.PlanOutTime >= {}  AND O.PlanOutTime <= {} "
queryorderinflowdata += "and O.CancelbyWLB = 0 group by P.BarCode,SQ.PositionCode order by SQ.PositionCode "
